...
class SimBlock(BaseModule):
    def __init__(self,
                 embed_dims,
                 conv_type='Conv2d',
                 # input_resolution=None,  # 特征图（h，w）
                 stride=1,
                 bias=True,
                 act_cfg=None,
                 init_cfg=None):
    ...
    def forward(self, x, hw_shape):
        ...
        # SIM 3*3卷积->归一化处理->高斯误差线性单元函数
        x = self.stage1(x)
        # SIM AvgPool_w和AvgPool_h
        x_h, x_w = self.pool_h(x), self.pool_w(x)  # .permute(0, 1, 3, 2)
        # SIM 矩阵相乘
        x = torch.matmul(x_h, x_w)
        # SIM 1*1卷积->归一化处理->高斯误差线性单元函数
        x = self.stage2(x)
        return x


@MODELS.register_module()
class SIMSwinTransformerPlus(BaseModule):
    """SIM + Swin Transformer backbone + ResNet backbone.
    ...
    Args:
        ...
        init_cfg (dict, optional): The Config for initialization.
            Defaults to None.
        is_sim: 使用SIM模块，Defaults to True.
    """
    ...
    def init_weights(self):
        if self.init_cfg is None:
            ...
            ckpt = CheckpointLoader.load_checkpoint(
                self.init_cfg['checkpoint'], logger='', map_location='cpu')
